...
async function apply(ctx, config) {
  ...
  async function probeModels(name) {
    ...
    try {
      ...
      const resp = await fetch(modelsUrl, { headers: buildHeaders(s.apiKey), signal: controller.signal })
      clearTimeout(timer)
      if (!resp.ok) {
  ...
  async function callDashScope(s, prompt, opts, allModels) {
    const model = opts.model || allModels[0]
    // DashScope 需要 X-DashScope-Async: enable + 任务轮询
    const createUrl = s.baseUrl.replace(/\/api\/v1\/?$/, '/api/v1/services/aigc/text2image/image-synthesis')
    ...
    // 轮询任务结果
    const taskUrl = s.baseUrl.replace(/\/api\/v1\/?$/, '/api/v1/tasks/') + taskId
    for (let i = 0; i < 20; i++) {
      await new Promise((r) => setTimeout(r, 2000))
      const pollResp = await fetch(taskUrl, { headers: { 'Authorization': `Bearer ${s.apiKey}` } })
  ...
  // ── agent 工具 ──
  // 使用 ESM 动态导入 defineTool（dsh-tools 是 ESM）
  let defineTool
  try {
  ...
  if (ctx.tools && defineTool) {
    ctx.tools.register(defineTool({
      ...
      output: {
        ...
        render: (result) => result.hint || '图片已生成',
      },
  ...
  // ── REST API ──
  // 默认源：优先 sensenova（已验证可用），再按配置顺序兜底
